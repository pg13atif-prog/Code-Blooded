...
const mapMovie = (movie) => ({
  ...
  rating: (movie.vote_average && Number(movie.vote_average) > 0) ? Number(movie.vote_average).toFixed(1) : 'N/A',
...
export const getMovieDetails = async (movieId, mediaType = 'movie', signal) => {
  ...
  return {
    ...
    rating: (data.vote_average && Number(data.vote_average) > 0) ? Number(data.vote_average).toFixed(1) : 'N/A',
